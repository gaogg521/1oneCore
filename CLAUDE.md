...
- **【2026-07-21 最新】aionrs 侧修复长 Write 工具调用截断后静默丢失(本仓仅 `Cargo.lock` 对齐新 commit `33c2bd22`,无源码改动)**：用户抓包实测定位；`openai_defaults()` 补 `default_max_tokens` + 新增 `LlmEvent::ToolCallTruncated` 事件让截断的工具调用可恢复重试，不再静默假装写完。dev 环境重编 + CDP 实测 glm-5-2 验证生效。⚠️验证中发现一个新坑（未修，留下一轮）：kimi-k3 超长请求会撞网关连接超时静默失败（与本次截断 bug 是不同机制）。详见 1oneUI 那份文档：
