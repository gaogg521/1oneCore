...
- **【2026-07-20 最新】三仓上游二次同步（本仓 v0.1.48→9 提交，`factory/aionrs.rs`/`manager/aionrs/agent.rs`/`aionui-db` 均有改动）**：跟上游 #641 撤销了 fork 自建的"按模型最大输出 Token"功能分支，`max_tokens` 全链路恒为 `None`（⚠️ 设置页那个"最大输出"输入框现在是死 UI，填了不生效，待下一轮清理或重新设计）；`Cargo.toml` 的 `aion-*` 依赖仍钉 `gaogg521/aionrs` `master`（未改上游裸 tag，fork 铁律不变）；migration 撞号 025→030；顺带修了 `aionui-db` 里两个已经红了 9 天的品牌断言 + 两处更早的迁移编号遗漏。详见 1oneUI 仓库这份文档（含 aionrs 侧的输出截断续写修复，是本次同步的直接起因）：
