...
impl ProviderService {
    ...
    /// appearing to vanish — see [`row_to_response`](Self::row_to_response),
    /// which this delegates to for that behavior.
    ...
    async fn ensure_not_company_managed(&self, user_id: &str, id: &str) -> Result<(), SystemError> {
        if let Some(row) = self.repo.find_by_id(user_id, id).await?
    ...
    pub async fn update(
        ...
        self.ensure_not_company_managed(user_id, id).await?;
    ...
    fn row_to_response(&self, row: Provider) -> Result<ProviderResponse, SystemError> {
        // Lenient on decryption for the same reason `list()` already is: a
        // credential encrypted under a rotated/lost key (e.g. one saved
        // during the ELECTRON-3T0 broken session, whose in-memory-only key is
        // gone forever) must not take this row down. Surface it with an
        // empty api_key + `Unrecoverable` status so the user can see it and
        // re-enter the key; any other decryption error still propagates.
        match decrypt_string(&row.api_key_encrypted, &self.encryption_key) {
            Ok(api_key) => self.build_response(row, api_key, ProviderKeyStatus::Ok),
            Err(crypto_err) if crypto_err.is_bad_request() => {
                ...
                self.build_response(row, String::new(), ProviderKeyStatus::Unrecoverable)
            }
            Err(crypto_err) => Err(SystemError::from(crypto_err)),
        }
    }
...
mod tests {
    ...
    async fn list_surfaces_undecryptable_provider_rows_as_unrecoverable() {
        let db = init_database_memory().await.unwrap();
        let repo: Arc<dyn aionui_db::IProviderRepository> = Arc::new(SqliteProviderRepository::new(db.pool().clone()));
        std::mem::forget(db);
