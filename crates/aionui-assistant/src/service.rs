...
impl AssistantService {
    ...
    async fn sync_legacy_user_assistant_to_new_tables_for_user(
        ...
        self.upsert_definition_from_legacy_user_row_for_user(user_id, row, None, "user")
            .await?;
        Ok(())
    }
    ...
    async fn upsert_definition_from_legacy_user_row_for_user(
        ...
        // `source` stays a parameter (not hardcoded "user"): the marketplace
        // installs personas as `imported`, and collapsing that to "user" here
        // is what previously made an imported persona indistinguishable from a
        // hand-written one.
        let (definition_id, assistant_id) = self
            .resolve_definition_identity_for_user(user_id, source, Some(&row.id), &row.id)
            .await?;
        ...
        self.definition_repo
            .upsert_for_user(
                ...
                &UpsertAssistantDefinitionParams {
                    id: &definition_id,
                    assistant_id: &assistant_id,
                    // Stays the caller's `source`, not a hardcoded "user":
                    // marketplace installs arrive as `imported`.
                    source,
                    owner_type: "user",
    ...
    pub async fn update_for_user(
        ...
        let row = self
            ...
            .await?
            .ok_or_else(|| AssistantError::NotFound(format!("assistant '{id}' not found")))?;
        self.upsert_definition_from_legacy_user_row_for_user(user_id, &row, requested_agent_id.as_deref(), "user")
            .await?;
        self.apply_detail_overrides_for_user(user_id, id, detail_overrides, reset_model_and_permission)
            .await?;
        if let Some(definition) = self.definition_repo.get_by_assistant_id_for_user(user_id, id).await? {
