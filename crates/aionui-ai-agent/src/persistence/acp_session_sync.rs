...
async fn domain_event_consumer(
    ...
    loop {
        ...
        match recv {
            Some(event) => {
                // `SessionIdDurable`, not `SessionAssigned`: an id is only worth
                // persisting once the CLI side actually has the session, which
                // is a strictly later moment (see the event's own doc comment —
                // persisting on assignment made every resume attempt rebuild a
                // session that was never going to be found).
