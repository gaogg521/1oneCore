...
- **Fork:** when the base URL is a custom/private OpenAI-compatible gateway (no catalog API hit), the resolution is `Supported` by default — a custom gateway fronts an operator-chosen alias (e.g. `glm-latest`, `deepseek-pro-latest`) that this catalog has no way to identify either way, and failing closed there silently strips every attached image with no error, even for a genuinely vision-capable backend model. A loose match to an allowlisted vision model ID short-circuits to `Supported` without needing this default. If the underlying model truly can't take images, the provider itself rejects the request — that failure is visible and actionable, unlike a client-side strip. (2026-08-17: prior to this, unmatched custom-gateway models defaulted to `Unknown`/fail-closed; that produced a silent, permanent vision blackout for any operator-aliased model name, which is exactly the "custom gateway is not fail-closed" behavior this note originally promised but the code did not implement for the *unmatched* case — only the *matched* case.)
