...
impl TeamAgentProvisioner {
    ...
    ///   backfills in migrations 003/044). So an agent that has never connected on
    ...
    async fn create_team_conversation_for_agent(
        ...
        let provider_id = if agent_type == AgentType::Aionrs {
            // Deliberately fails fast rather than upstream's fall back to the
            // backend label: a teammate whose model no provider offers would
            // otherwise be provisioned against a provider_id that cannot serve
            // it, and only fail later at send time with an opaque error.
