...
async fn setup_fork() -> (
    ...
    let acp_session_repo: Arc<dyn aionui_db::IAcpSessionRepository> =
        Arc::new(aionui_db::SqliteAcpSessionRepository::new(db.pool().clone()));
    let task_mgr: Arc<dyn IWorkerTaskManager> = Arc::new(NoopTaskManager);
    // ⚠️ FORK DIVERGENCE from upstream.
    //
    // Upstream's migration 036 hand-seeds `session_capabilities.fork` onto the
    // builtin claude/codex rows, because upstream routes them as direct-CLI
    // connections that never perform an ACP handshake — so nothing can ever
    // overwrite a constructed value there.
    //
    // On this fork `route_for_backend` deliberately maps claude/codex to
    // `BackendRoute::AcpManager` (to keep the bridge env injection; there are
    // tests locking that down), so they DO handshake, and `apply_handshake`
    // REPLACES `agent_capabilities` wholesale. A constructed seed would either
    // be wiped on the next connect or advertise a surface the live agent may
    // not have — so `048_conversation_fork.sql` deliberately omits it and the
    // live handshake is the single source of truth.
    //
    // These tests are about the fork SERVICE logic, not about where the
    // capability came from, so the fixture supplies it through the exact
    // mechanism this fork uses in production: a persisted ACP handshake.
    for (agent_id, fork_shape) in [
        // claude: HEAD-only fork (`--resume <sid> --fork-session`).
        ("2d23ff1c", r#"{"fork":{}}"#),
        // codex: fork at an arbitrary turn (`thread/fork {lastTurnId}`).
        ("8e1acf31", r#"{"fork":{"at_turn":true}}"#),
    ] {
        let capabilities = format!(r#"{{"session_capabilities":{fork_shape}}}"#);
        agent_metadata_repo
            .apply_handshake(
                agent_id,
                &aionui_db::UpdateAgentHandshakeParams {
                    agent_capabilities: Some(Some(&capabilities)),
                    ..Default::default()
                },
            )
            .await
            .unwrap()
            .expect("seeded builtin agent row");
    }
...
async fn fork_head_creates_lineage_row_and_copies_messages() {
    ...
    // HEAD fork (claude declares a head-only fork capability; see setup_fork).
