...
pub trait IMcpServerRepository: Send + Sync {
    ...
    ///
    /// This default body goes through [`find_by_id_any`](Self::find_by_id_any),
    /// which does NOT itself exclude soft-deleted rows — implementors that
    /// override this method (e.g. the SQLite repo, which filters
    /// `deleted_at IS NULL` directly in SQL) must preserve that exclusion.
