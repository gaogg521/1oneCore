//! Idempotent pre-migration repair for migration 042 (`user_scope`).
//!
//! Migration 042 asserts several data-integrity invariants via fail-hard
...
//! ELECTRON-31X).
//!
//! We cannot rewrite the shipped 042 (sqlx checksum / VersionMismatch) and a
//! forward catch-up migration cannot run (the migrator stops at the first
//! failing migration), so we normalize the raw database to satisfy 042's
//! invariants *before* the migrator runs 042 — reusing the migrator's
...
pub(crate) const USER_SCOPE_MIGRATION_VERSION: i64 = 42;
...
/// True iff migrations 001–041 are all applied and 042 is the next pending
/// migration (`max applied version == 41`). Returns false when the
...
/// Evaluate every named 042 invariant on the raw (pre-042) schema and return
...
async fn evaluate_user_scope_guards(conn: &mut sqlx::SqliteConnection) -> Result<Vec<GuardViolation>, DbError> {
    ...
    for (check, count_sql) in GUARD_COUNT_QUERIES {
        match sqlx::query_scalar::<_, i64>(count_sql).fetch_one(&mut *conn).await {
            ...
            // v41 database every referenced object exists (migrations 001–041),
...
/// v41 database every referenced object exists (migrations 001–041), so this
...
/// (check_name, COUNT(*) query) pairs mirroring 042's guard predicates.
/// Filled in by Task 2 (A-class) and Task 3 (C-class + system_settings).
const GUARD_COUNT_QUERIES: &[(&str, &str)] = &[
    // --- A-class: orphan child rows (042_user_scope.sql:75-158, 787-810) ---
    (
    ...
    // --- C-class: duplicate composite-UNIQUE identities (042:225-263, 741-783) ---
    (
    ...
    // --- L4: system_settings row not keyed at id=1 would be dropped by 042 (042:318) ---
    (
        "system_settings_non_primary_id",
        "SELECT COUNT(*) FROM system_settings WHERE (SELECT COUNT(*) FROM system_settings) = 1 AND id <> 1",
    ),
    // --- B-class (DIAGNOSTIC-ONLY, no active repair): assistant_sessions cross-scope owner (042:812-824). ---
    // 042 forces assistant_users.owner_user_id='system_default_user' (042:768) but never normalizes
    // conversations.user_id, so 042's `c.user_id != u.owner_user_id` guard fails for a session-linked
...
pub(crate) async fn repair_user_scope_preconditions(conn: &mut sqlx::SqliteConnection) -> Result<(), DbError> {
    ...
    // Post-repair re-check: re-evaluate the same invariants so a subsequent 042
    ...
    // fully cover) instead of 042's generic `CHECK constraint failed: ok = 1`.
    // This closes the "repaired vs. never-covered" ambiguity the pre-check alone
    // cannot resolve: a residual named here is exactly what will trip 042 next.
    ...
    if residual.is_empty() {
        ...
        for v in &residual {
            warn!(
                ...
                check = v.check,
                count = v.count,
                "user_scope invariant still violated after repair; migration 042 may fail"
            );
        }
    }
    Ok(())
...
/// On a real v41 database every referenced object exists, so nothing is skipped.
async fn apply_user_scope_repairs(conn: &mut sqlx::SqliteConnection) -> Result<(), DbError> {
    ...
    // key so 042's full-table rebuild INSERT does not hit UNIQUE(user_id,name)
    // / UNIQUE(owner_user_id,platform_user_id,platform_type). (042:241, 763.)
    // mcp_servers: all legacy rows get user_id=system_default_user in 042, so
    ...
    // L4: if exactly one system_settings row exists and it is not id=1, 042's
...
mod tests {
    ...
    async fn create_min_v41_aggregate_tables(c: &mut sqlx::SqliteConnection) {
        for ddl in [
    ...
    async fn a_class_deletes_orphans_and_keeps_valid() {
        let mut c = conn().await;
        create_min_v41_aggregate_tables(&mut c).await;
    ...
    async fn c_class_promotes_lone_system_settings_row_to_id_1() {
        ...
        assert_eq!(
            id, 1,
            "lone settings row promoted to id=1 so 042 WHERE id=1 copy preserves it"
        );
    }
